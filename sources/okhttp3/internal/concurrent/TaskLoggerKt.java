...
public final class TaskLoggerKt {
    ...
    public static final <T> T logElapsed(Task task, TaskQueue taskQueue, Function0<? extends T> function0) {
        ...
        try {
            ...
            if (isLoggable) {
                log(task, taskQueue, Intrinsics.stringPlus("finished run in ", formatDuration(taskQueue.getTaskRunner$okhttp().getBackend().nanoTime() - j)));
            }
            ...
            InlineMarker.finallyStart(1);
            if (isLoggable) {
                log(task, taskQueue, Intrinsics.stringPlus("failed a run in ", formatDuration(taskQueue.getTaskRunner$okhttp().getBackend().nanoTime() - j)));
            }
            InlineMarker.finallyEnd(1);
            throw th;
        }
