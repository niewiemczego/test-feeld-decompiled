...
public abstract class MeasurementManager {
    ...
    private static final class Api33Ext5Impl extends MeasurementManager {
        ...
        private final List<WebSourceParams> convertWebSourceParams(List<WebSourceParams> list) {
            List<WebSourceParams> arrayList = new ArrayList<>();
            for (WebSourceParams next : list) {
                WebSourceParams build = new WebSourceParams.Builder(next.getRegistrationUri()).setDebugKeyAllowed(next.getDebugKeyAllowed()).build();
        ...
        public final WebTriggerRegistrationRequest convertWebTriggerRequest(WebTriggerRegistrationRequest webTriggerRegistrationRequest) {
            WebTriggerRegistrationRequest build = new WebTriggerRegistrationRequest.Builder(convertWebTriggerParams(webTriggerRegistrationRequest.getWebTriggerParams()), webTriggerRegistrationRequest.getDestination()).build();
            Intrinsics.checkNotNullExpressionValue(build, "Builder(\n               …\n                .build()");
            return build;
        }

        private final List<WebTriggerParams> convertWebTriggerParams(List<WebTriggerParams> list) {
            List<WebTriggerParams> arrayList = new ArrayList<>();
            for (WebTriggerParams next : list) {
                WebTriggerParams build = new WebTriggerParams.Builder(next.getRegistrationUri()).setDebugKeyAllowed(next.getDebugKeyAllowed()).build();
                Intrinsics.checkNotNullExpressionValue(build, "Builder(param.registrati…                 .build()");
                arrayList.add(build);
            }
            return arrayList;
