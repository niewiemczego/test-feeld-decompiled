...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public int consumeFlingInVerticalStretch(int i) {
        ...
        if (i > 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowTop) != 0.0f) {
            int round = Math.round((((float) (-height)) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (((float) (-i)) * FLING_DESTRETCH_FACTOR) / ((float) height), 0.5f));
            if (round != i) {
            ...
            int round2 = Math.round((f / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, (((float) i) * FLING_DESTRETCH_FACTOR) / f, 0.5f));
            if (round2 != i) {
    ...
    private float getSplineFlingDistance(int i) {
        double log = Math.log((double) ((((float) Math.abs(i)) * 0.35f) / (this.mPhysicalCoeff * SCROLL_FRICTION)));
