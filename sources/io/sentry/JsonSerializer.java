...
public final class JsonSerializer implements ISerializer {
    ...
    public <T, R> T deserializeCollection(Reader reader, Class<T> cls, JsonDeserializer<R> jsonDeserializer) {
        ...
        try {
            jsonObjectReader = new JsonObjectReader(reader);
            if (!Collection.class.isAssignableFrom(cls)) {
                T nextObjectOrNull = jsonObjectReader.nextObjectOrNull();
                jsonObjectReader.close();
                return nextObjectOrNull;
            } else if (jsonDeserializer == null) {
                T nextObjectOrNull2 = jsonObjectReader.nextObjectOrNull();
                jsonObjectReader.close();
                return nextObjectOrNull2;
            } else {
                T nextListOrNull = jsonObjectReader.nextListOrNull(this.options.getLogger(), jsonDeserializer);
                jsonObjectReader.close();
                return nextListOrNull;
            }
        } catch (Throwable th) {
    ...
    public <T> T deserialize(Reader reader, Class<T> cls) {
        ...
        try {
            ...
            this.options.getLogger().log(SentryLevel.ERROR, "Error when deserializing", (Throwable) e);
            return null;
        } catch (Throwable th) {
            th.addSuppressed(th);
        }
    ...
    public SentryEnvelope deserializeEnvelope(InputStream inputStream) {
        ...
        try {
            ...
            this.options.getLogger().log(SentryLevel.ERROR, "Error deserializing envelope.", (Throwable) e);
            return null;
        }
    }
    ...
    public <T> void serialize(T t, Writer writer) throws IOException {
        ...
        if (this.options.getLogger().isEnabled(SentryLevel.DEBUG)) {
            String serializeToString = serializeToString(t, this.options.isEnablePrettySerializationOutput());
            this.options.getLogger().log(SentryLevel.DEBUG, "Serializing object: %s", serializeToString);
        }
    ...
    public void serialize(SentryEnvelope sentryEnvelope, OutputStream outputStream) throws Exception {
        ...
        try {
            ...
            for (SentryEnvelopeItem next : sentryEnvelope.getItems()) {
                ...
                next.getHeader().serialize(new JsonObjectWriter(bufferedWriter, this.options.getMaxDepth()), this.options.getLogger());
                bufferedWriter.write("\n");
            ...
            this.options.getLogger().log(SentryLevel.ERROR, "Failed to create envelope item. Dropping it.", (Throwable) e);
        } catch (Throwable th) {
            bufferedWriter.flush();
            throw th;
        }
