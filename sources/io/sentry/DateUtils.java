...
public final class DateUtils {
    ...
    public static Date getDateTime(long j) {
        Calendar instance = Calendar.getInstance(ISO8601Utils.TIMEZONE_UTC);
        instance.setTimeInMillis(j);
        return instance.getTime();
    }

    public static Date nanosToDate(long j) {
        return getDateTime(Double.valueOf(nanosToMillis(Double.valueOf((double) j).doubleValue())).longValue());
    }
    ...
    public static double nanosToSeconds(long j) {
        return Double.valueOf((double) j).doubleValue() / 1.0E9d;
    }
