...
public final class KotlinExtensions$await$2$2 implements Callback<T> {
    ...
    public void onResponse(Call<T> call, Response<T> response) {
        ...
        Intrinsics.checkParameterIsNotNull(response, Response.TYPE);
        if (response.isSuccessful()) {
            ...
            if (body == null) {
                ...
                this.$continuation.resumeWith(Result.m983constructorimpl(ResultKt.createFailure(kotlinNullPointerException)));
                return;
            }
            Result.Companion companion2 = Result.Companion;
            this.$continuation.resumeWith(Result.m983constructorimpl(body));
            return;
        }
        Result.Companion companion3 = Result.Companion;
        this.$continuation.resumeWith(Result.m983constructorimpl(ResultKt.createFailure(new HttpException(response))));
    }

    public void onFailure(Call<T> call, Throwable th) {
        Intrinsics.checkParameterIsNotNull(call, NotificationCompat.CATEGORY_CALL);
        Intrinsics.checkParameterIsNotNull(th, Constants.BRAZE_PUSH_TITLE_KEY);
        Result.Companion companion = Result.Companion;
        this.$continuation.resumeWith(Result.m983constructorimpl(ResultKt.createFailure(th)));
    }
}
